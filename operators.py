...
def _find_all_legal_insertions(solution: Solution, order: Order, dist_matrix, customer_pool):
    legal_insertions = []
    ...
    for r_idx, route in enumerate(solution.routes):
        ...
        if curr_w + o_w > route.vehicle.capacity_weight or curr_v + o_v > route.vehicle.capacity_volume: continue
        for sub_idx, node_id in enumerate(route.nodes[1:-1]):
            if node_id == order.customer_id:
                temp_route = copy.copy(route); temp_route.orders = [list(s) for s in route.orders]
                temp_route.orders[sub_idx].append(order)
                new_cost = calc_route_total_cost(temp_route, dist_matrix_safe, customer_pool)
                if _is_time_table_safe(solution, route, temp_route):
                    legal_insertions.append((r_idx, sub_idx, True, new_cost - route.cost))
        for i in range(len(route.nodes) - 1):
            temp_route = copy.copy(route); temp_route.nodes = route.nodes[:i+1] + [order.customer_id] + route.nodes[i+1:]
...
def greedy_repair(solution: Solution, dist_matrix, customer_pool, vehicle_pool=None):
    ...
    while solution.unassignedOrders:
        ...
        for o_idx, order in enumerate(batch):
            insertions = _find_all_legal_insertions(solution, order, dist_matrix, customer_pool)
            if insertions:
                bi = min(insertions, key=lambda x: x[3])
                if bi[3] < min_delta: min_delta, best_candidate = bi[3], (o_idx, bi[0], bi[1], bi[2], False)
            if vehicle_pool:
                bv, nd = _try_open_new_route(solution, order, vehicle_pool, dist_matrix, customer_pool)
        ...
        if best_candidate:
            ...
            if is_new:
                ...
                if is_merge: route.orders[pos].append(order)
                else: route.nodes.insert(pos + 1, order.customer_id); route.orders.insert(pos, [order])
            _rebuild_maps_and_costs(solution, dist_matrix, customer_pool)
        else: break
...
def regret_repair(solution: Solution, dist_matrix, customer_pool, vehicle_pool=None, k_regret=2):
    ...
    while solution.unassignedOrders:
        ...
        for o_idx, order in enumerate(batch):
            poss = []
            ins = _find_all_legal_insertions(solution, order, dist_matrix, customer_pool)
